...
```
The suite definition lives in `src/probe/main_rollout_stats_suite.py` (shared `Qwen/Qwen3-1.7B` contract, `temperature=0.2`, `num_generations=10`, `max_tokens=81920`, `max_model_len=40960`, `max_num_seqs=10`, `max_num_batched_tokens=4096`). The launcher forwards `CONDA_ENV` / `CONDA_DEFAULT_ENV` into every submitted job and passes `THINKING_MODE` through explicitly — do not re-reintroduce ad-hoc thinking flags. `LiveCodeBench-extra` is intentionally removed; it is a strict subset of the `release_v6` `LiveCodeBench` run.
