...
The active execution object on the rollout-statistics line is the paired thinking `on` / `off` four-dataset rebuild described in `docs/weeks/2026-W17/main-four-dataset-rollout-rebuild-2026-04-23.md`: `LiveCodeBench`, `TACO-hard`, `MATH level-5`, and `Omni-MATH >= 7`, each collected twice under a shared `Qwen/Qwen3-1.7B` contract with reusable prompt-rollout archives. The sizing rule is full dataset when it is under `1000`, otherwise `1000`, except retained `LiveCodeBench` `release_v6`, which intentionally uses the full `1055` prompts. Earlier March repairs, the `LiveCodeBench`-only reruns, and the mistaken `LiveCodeBench-extra` lane are historical debugging context — do not treat them as the current stage contract.

Before making non-trivial changes, read `roadmap.md`, `backlog.md`, `docs/weeks/2026-W17/main-four-dataset-rollout-rebuild-2026-04-23.md`, `docs/weeks/2026-W13/prompt-profile-eval-contract.md`, and `docs/weeks/2026-W14/understand-where-loop-and-max-length-come-from.md`. These hold the current suite contract, the current split/target decisions (regression on `mean_relative_length`, binary `majority_s_0.5`), and the locked prompt-profile eval contract — do not redefine metrics, splits, or the rebuild dataset list ad hoc.

## Standing TODOs

`docs/standing/todos-2026-W17.md` is the active standing TODO ledger for the current rebuild. When given a new task that changes active project work, update the relevant standing TODO entry before or while starting the task. When the task is completed, paused, or its status changes, update the same entry so the ledger reflects the current state. Keep standing TODOs simple: track current active workstreams, not historical notes.

## Rollout artifact hygiene

Delete stale failed launch artifacts after recording the failure in the standing TODO or relevant run note. A stale failed launch is a job, dry-run, or canceled duplicate that failed before producing real prompt rows or a valid `rollout_bundle.v1` pair, such as bad sbatch/env setup, missing cache/checkout, failed conda activation, or launcher dry-run manifests.

Do not treat mid-run partials the same way. If a rollout fails after generating some prompts, preserve its partial bundle/logs/checkpoints for diagnosis or resume unless the user explicitly asks to remove them. Partial data can explain the failure mode and may be recoverable; startup-stale artifacts only add queue/log clutter. Before deleting anything, verify the current queue and active output root so running jobs and the latest intended manifest/logs are not removed.

## Rollout CPU finalization

Do not run CPU-only post-hoc grading inside GPU Slurm allocations. `livecodebench_codegen` and `taco_codegen` collection jobs should set `DEFER_CPU_FINALIZE=1`, exit after GPU generation, and leave a deferred sidecar. Finalize those bundles outside Slurm with GPUs hidden, for example `CUDA_VISIBLE_DEVICES='' uv run python scripts/rollout/collect_model_stats.py --task-kind livecodebench_codegen --out <bundle-base>.json --livecodebench-repo <repo> --finalize-only`. If a deferred GPU job is canceled after writing rows, resume with `RESUME=1`; the collector can reuse both final bundles and `__rank*.jsonl.gz` partials.
...
Train a probe:
```
python scripts/train/train_probe.py --data-dir <probe_data_dir> --out-dir outputs/probe_runs/<run> \
...
```
python scripts/train/eval_probe_checkpoint.py --checkpoint <run>/best.pt --data-dir <probe_data_dir> --split test
```
...
```
The suite definition lives in `configs/rollout/main_rollout_stats_suite.json` (shared `Qwen/Qwen3-1.7B` contract, `temperature=0.2`, `num_generations=10`, `max_tokens=81920`, `max_model_len=40960`, `max_num_seqs=10`, `max_num_batched_tokens=4096`). `src/probe/main_rollout_stats_suite.py` loads that config and translates it into collector environments. The launcher forwards `CONDA_ENV` / `CONDA_DEFAULT_ENV` into every submitted job and passes `THINKING_MODE` through explicitly — do not re-reintroduce ad-hoc thinking flags. `LiveCodeBench-extra` is intentionally removed; it is a strict subset of the `release_v6` `LiveCodeBench` run.
...
One linear flow, each stage owned by a module in `src/probe/`:
...
The current headline probe surface is **ensemble, `h256 d1`, on the balanced-binary `majority_s_0.5` object**. Regression runs on the natural prompt-disjoint split with natural sampling. Don't mix those two regimes by accident — see `docs/weeks/2026-W13/prompt-profile-eval-contract.md`.

### Adapters

`src/probe/adapters/` holds per-benchmark rollout adapters (MATH free-form, GPQA MC, MMLU-Pro MC, LiveCodeBench codegen). When adding a dataset family, add an adapter here rather than branching inside the collector — the rollout-statistics reports rely on consistent adapter output.

### Collector and rollout-stats suite

`collector.py` is the repaired v2 rollout-statistics path. Cross-dataset reports (`scripts/report_generation/build_cross_dataset_rollout_report.py`, `scripts/report_generation/build_model_rollout_report.py`) consume its output. If you touch the rollout schema, both the collector contract and those report builders need to stay in sync.

`configs/rollout/main_rollout_stats_suite.json` is the canonical four-dataset suite scaffold that the current rebuild uses (paired thinking `on` / `off`, shared sampling config). `src/probe/main_rollout_stats_suite.py` is the loader/translator for that config, and `scripts/rollout/launch_main_rollout_stats_suite.py` is the sbatch wrapper. Every run on a dataset emits exactly one `rollout_bundle.v1` pair: `<base>.jsonl.gz` with one row per prompt (full replay: `record_id`, raw `prompt` text + `prompt_token_ids`, `record_metadata`, and a `rollouts[*]` list with `completion_text`, `completion_token_ids`, loop / cap / tail flags, and task-specific `grading`), and a small `<base>.json` sidecar with run metadata + aggregate counts + metrics + `lcb_native_metrics`. No `__lcb_records.json` / `__prompt_rollout_archive.jsonl` / `__rollout_archive.jsonl.gz` / `__prompt_profile.jsonl` / `__progress.json` / `__lcb_records_checkpoint.json` sidecars exist anymore — all I/O routes through `src/probe/bundle_io.py`, and legacy bundles must first be run through `scripts/rollout/migrate_legacy_rollout_bundle.py`. See [docs/reference/rollout-bundle-v1-schema.md](docs/reference/rollout-bundle-v1-schema.md) for the canonical shape. This bundle is load-bearing downstream: the same rollouts drive prompt-profile relabeling, probe training, and steering without re-running the GPU work. Two runtime corrections are baked into the suite and must stay that way: the TACO native grader returns top-level callables directly (it does not rebind them as `Solution` methods), and `BAAI/TACO` is loaded via the HF parquet surface `hf://datasets/BAAI/TACO/ALL/<split>-*.parquet`, not the retired `TACO.py` dataset script.

### Scripts vs library

- `src/probe/` — reusable, no CLI.
- `src/steer/` — steering-stage registries and artifact-record helpers.
...
Defined in `src/probe/configs.py` (`MODEL_ROLLOUT_DEFAULTS`): `qwq_32b` (tp=8), `openthinker3_7b` (dp=8), `openthinker3_1p5b` (dp=8). Override any field via CLI (`--temperature`, `--max-tokens`, `--tp`, `--dp`, `--max-num-seqs`, …) or skip presets entirely with `--model-id`.
...
- Keep utilities and CLI entry points in separate files (utilities in `src/probe/` or `scripts/utils.py`, entry points in `scripts/*.py`).
- Output naming is load-bearing: run folders under `outputs/probe_runs/`, metrics as `*_metrics.csv`, checkpoints `best.pt` / `last.pt`, per-epoch `metrics.jsonl`, figures under `outputs/`. Report-style artifacts live in dated folders (e.g. `outputs/weeks/2026-W14/prompt_profile_combined_audit_20260405/`).
