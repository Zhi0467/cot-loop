...
- Prompt-profile direction: the live in-distribution objective is now narrower than the original `s_0.9` / `mean_relative_length` fork. A 2026-03-22 Athena consult was grounded in the repaired rollout-stat bundle, the saved GPQA prompt-profile archive, and the current code constraints, and it matched the local read: the next shipped scalar head should be direct prompt-level loop probability `p_loop`, not another tail proxy. That recommendation now also matches the saved GPQA archive geometry: `p_loop` is denser than `p_cap`, correlates strongly with `mean_relative_length` (`Spearman 0.849`), and is less tied to prompt length (`0.297`) than `mean_relative_length` (`0.375`). The repo now supports `--target-kind probability --profile-target p_loop|p_cap`, so the best zero-plumbing fallback stays `mean_relative_length`, while `p_cap` remains a sharper but narrower diagnostic companion. Prompt-token-count and effective budget `E` remain the leakage baselines, and any single-layer probe should still default to the last layer.
