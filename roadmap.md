...
- Active experiment state: the next all-dataset pilot is now queued on the remote node rather than only described in docs. The branch now supports direct prompt-majority binary targets (`majority_s_t`) plus a `LiveCodeBench` prompt-loader path, and the submission chain `1492`-`1505` is staged serially on the pinned 2-GPU pair (`3,5`). Job `1492` (`GPQA`, last-layer, `158 / 40`) is already running and has passed prefill startup into repeated rollouts.
