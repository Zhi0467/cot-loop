...
- Active experiment state: the old `2`-GPU prompt-majority chain was canceled because the thread tightened the compute cap to `1` GPU. The branch now has a dedicated prompt-level projection exporter / renderer plus a `1`-GPU Slurm entrypoint (`run_prompt_profile_projection.sbatch`). The corrected serial all-dataset visualization chain is `1539`-`1543` (`GPQA` -> `AIME` -> `MATH-500` -> `MMLU-Pro` -> `LiveCodeBench`), and `1539` is already running on `wth-gpu-01`.
