...
## Project Focus
This repository builds a chain-of-thought (CoT) failure predictor from configurable activation views plus the paired rollout-statistics line that explains where degenerate rollouts come from. The older target-choice debate is closed: regression `mean_relative_length` and binary `majority_s_0.5` remain the locked prompt-profile heads, but the missing collaborator-requested artifact is specifically the balanced-train / natural-test regression rerun on the saved April prompt subset. In parallel, the OLMo degeneration-origin thread is now an audit object, not a blind scale-up object: `LiveCodeBench` needs a model-native wrapper and `RLVR / MMLU-Pro = 0 / 80` still needs explanation before anyone should make a stronger stage claim. Before launching work, read `roadmap.md`, `backlog.md`, `docs/prompt-profile-eval-contract.md`, `docs/prompt-profile-risk-screen-2026-03-30.md`, and `understand-where-loop-and-max-length-come-from.md`.
