...
Runtime cache rule: all Hugging Face model and dataset caches used by Slurm entrypoints must resolve through `slurm/cache_env.sh` to `/data/shared/huggingface` (`HF_HOME`, hub cache, Transformers cache, datasets cache, and assets cache). Do not put HF artifacts under `/data/scratch/${USER}/cache` or `/data/users/${USER}/cache`.
