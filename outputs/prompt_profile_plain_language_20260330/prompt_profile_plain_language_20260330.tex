...
The project goal is \textbf{not} ``rank prompts.'' The project goal is: choose
the right prompt-level label or regression target to predict from
prompt-prefill activations under one fixed model and one fixed decode policy.
The \texttt{top 20\%} bucket was only a common diagnostic I used to compare
candidate targets on the same footing.

\section*{One-sentence answer}
On the current saved five-dataset bundle, the best main target is
\texttt{p\_loop}: for each prompt, regress the fraction of rollouts that enter
the loop regime.

\section*{What the project is actually trying to do}
Fix one \texttt{(model, decode policy)} pair. For each prompt, define one
prompt-level target from repeated rollouts. Then ask which target is the best
thing to learn from prompt-prefill activations. ``Best'' here means the learned
score should track the bad behavior we actually care about, not just prompt
geometry or budget usage.

So the project question is a target-choice question:
\begin{itemize}[leftmargin=1.5em]
\item should we train on \texttt{majority\_s\_0.5}?
\item should we regress \texttt{p\_loop}?
\item should we regress \texttt{mean\_relative\_length}?
\item should we regress \texttt{p\_cap}?
\end{itemize}

On the current saved five-dataset bundle, the answer is \texttt{p\_loop}.
...
\textbf{Flagged top-risk bucket}: this phrase only refers to an evaluation
slice. For one trained model on one held-out test split, take the \texttt{20\%}
of prompts with the largest predicted badness score, then check what really
happened on those prompts. It is not a new project goal and it is not a new
label.

\textbf{Loop-rate enrichment}: loop rate inside the flagged bucket, divided by
the overall loop rate on the test split. A value of \texttt{2.0x} means the
bucket has twice the average loop rate. A value near \texttt{1.0x} means no
concentration.

\textbf{Why use loop-rate enrichment?} Different datasets have very different
base loop rates, and the candidate targets live on different scales
(\texttt{majority\_s\_0.5} is binary; \texttt{p\_loop} and \texttt{p\_cap} are
probabilities; \texttt{mean\_relative\_length} is continuous). Enrichment gives
one simple common check: when a target says ``these prompts look bad,'' do those
prompts actually loop more often than usual?
...
\item a common held-out bucket test: take the \texttt{20\%} of prompts with the
largest predicted badness score, and measure the actual loop rate, cap-hit
rate, and accuracy inside that bucket.
\end{enumerate}
...
\item take the \texttt{20\%} of prompts with the largest predicted badness
score;
...
This bucket test is only one evaluation device. I used it because it lets
different target types share one simple held-out check without choosing a
custom threshold for every target.
...
\texttt{p\_loop} bucket is also the lowest-accuracy bucket every time.

If the project goal is ``choose the prompt-level target that best matches real
looping behavior,'' then \texttt{p\_loop} is the most aligned target: it
directly asks how often the prompt loops, it consistently concentrates
loop-heavy prompts, and it also concentrates low-accuracy prompts where
...
especially \texttt{AIME}. So it should stay as a control or cheap auxiliary
screen, not the main target.

\textbf{Why not \texttt{mean\_relative\_length}}: it is a useful dense target and
tracks budget usage well, but it mixes correct long reasoning, wrong long
...
The main reason is alignment between the target and the thing we actually want
to predict:
\begin{itemize}[leftmargin=1.5em]
...
\item training \texttt{p\_loop} teaches the event we actually want to predict:
entering the loop regime.
\end{itemize}

So the winning result is not mysterious. The target itself matches the failure
mode better, and that survives the held-out comparison.
...
continuous heads;
\item it is no longer missing the metadata-only baseline pass;
\item it is no longer missing one common held-out comparison slice.
\end{itemize}
