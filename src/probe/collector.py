...
class WorkerAggregator:
    """Numeric counters for a single run.

    Per-rollout rows live in the ``rollout_bundle.v1`` ``<base>.jsonl.gz`` file
    written through :mod:`probe.bundle_io`; this struct only carries the
    aggregate counters that feed the sidecar ``<base>.json`` metrics.
    """
