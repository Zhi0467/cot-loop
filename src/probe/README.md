...
- `types.py`: shared dataset/sample records.
- `prompt_format.py` / `prompt_builder.py`: model prompt formatting.
...
- `rfm.py`: Laplace RFM helper used by prompt-profile detector runs.
- `train_utils.py`: seed/device/metrics helpers.

Steering-specific registry and artifact helpers live in `src/steer/`.
