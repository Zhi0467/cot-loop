...
Update:
- the larger OLMo2 fallback follow-up now lives at `docs/weeks/2026-W14/olmo2-1b-fifty-prompt-rerun-2026-04-05.md`;
...
This note closes the bounded OLMo audit that started from `docs/weeks/2026-W14/understand-where-loop-and-max-length-come-from.md`.
...
1. `RLVR / MMLU-Pro = 0 / 80` was a grader bug.
   ...
   - `_extract_json_answer_field` in `src/probe/adapters/_common.py` now accepts that form.
...
2. `SFT / LiveCodeBench = 0 / 80` was initially on the wrong wrapper surface.
   ...
   - `src/probe/adapters/livecodebench_codegen.py` now has an OLMo-native path:
...
- Report PDF: `outputs/weeks/2026-W14/olmo_degeneration_origin_audit_20260404/olmo_degeneration_origin_audit_20260404.pdf`
...
- the older Qwen reference object it is being compared against is the repaired v2 bundle `outputs/undated/qwen3_1p7b_rollout_stats_v2_temp0p2_gen10/`, with the long-horizon contract `temperature=0.2`, `num_generations=10`, `max_tokens=81920`, and `max_model_len=40960`;
