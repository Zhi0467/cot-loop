...
3. Relabel the same archive to the locked binary head.
   - Use `scripts/data/relabel_prompt_profile_dataset.py`.
   - `--target-kind binary`
...
The canonical entrypoint for that chain is now `scripts/train/run_prompt_profile_full_train.py`. It freezes the dataset table below in code and can also hand the finished run roots to `scripts/report_generation/summarize_prompt_profile_full_train.py` for the locked-pair ledger plus metadata baselines.
...
```bash
python scripts/data/relabel_prompt_profile_dataset.py \
