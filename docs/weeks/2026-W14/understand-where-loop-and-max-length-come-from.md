...
The rollout-level loop bit is defined in `src/probe/labeling.py`.
...
- it is defined in `src/probe/labeling.py`;
...
This is the older `max_length_hit` object used by `scripts/rollout/collect_model_stats.py` and the cross-dataset rollout reports.
...
In `scripts/data/build_probe_dataset.py`, for each prompt:
...
The repeated-rollout path in `scripts/data/build_probe_dataset.py` writes three relevant layers of state.
...
- `docs/weeks/2026-W12/prompt-profile-projection.md` describes the prompt-level projection path.
- `docs/weeks/2026-W12/prefill-activation-visualization.md` describes the earlier rollout/prompt visualization path.
...
- `outputs/undated/qwen3_1p7b_cross_dataset_rollout_stats/cross_dataset_rollout_summary.csv`

Important:

- in this table, `max_length_hit` is the rollout-stat object from `scripts/rollout/collect_model_stats.py`;
...
From `docs/weeks/2026-W12/prefill-activation-visualization.md` on the saved GPQA pilot:
