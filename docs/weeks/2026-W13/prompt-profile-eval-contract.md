...
- on the finished five-dataset analysis bundle (`outputs/weeks/2026-W14/prompt_profile_risk_controls_20260330/`), the selected `p_loop` screen wins loop-rate enrichment on `AIME`, `MATH-500`, `MMLU-Pro`, and `LiveCodeBench`, while roughly tying `GPQA` with `majority_s_0.5`;
