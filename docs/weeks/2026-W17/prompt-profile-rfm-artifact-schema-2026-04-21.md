...
> **Schema migration note (2026-04-23):** `scripts/rollout/collect_model_stats.py` now
> emits a single `rollout_bundle.v1` pair (`<base>.jsonl.gz` +
> `<base>.json`) per run; see
> [docs/reference/rollout-bundle-v1-schema.md](../../reference/rollout-bundle-v1-schema.md). The
> `prompt_profile_file`, `prompt_rollout_archive_file`, `__progress.json`, and
> `__lcb_records_checkpoint.json` sidecars referenced below are legacy names;
> the equivalent data now lives inline in the bundle row (`prompt_profile`,
> `prompt` / `prompt_token_ids`, `rollouts[*]`, `grading`) and the resume
> contract is "scan the partial `<base>.jsonl.gz`" instead of a separate
> progress file. Stage-0.5 / RFM provenance records produced by the
> probe-dataset side (`build_probe_dataset.py`) are unchanged.
...
- Current implementation surface:
  - aggregate JSON written by `scripts/rollout/collect_model_stats.py`
...
- Supporting record: `split_manifest.json` emitted by `scripts/data/materialize_prompt_profile_stage_binary_data.py`
- Required keys:
  - `benchmark`
  - `prompt_ids.train`
  - `prompt_ids.val`
...
- Shared helpers live in `src/steer/stage_artifacts.py`.
- Registry emission and validation live in:
  - `scripts/steer/emit_prompt_profile_rfm_stage_registry.py`
  - `scripts/steer/validate_prompt_profile_rfm_stage_registry.py`
- The stage registry itself lives in `src/steer/prompt_profile_rfm_stage_registry.py`.
