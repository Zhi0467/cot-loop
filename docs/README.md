...
- The current design recommendation is to move the next predictor round away from another binary loop label and away from raw `p(max_length_hit)` as the sole main target. The live objective is now a prompt-level rollout-profile target trained directly on repeated-rollout labels: `p_loop` first, `mean_relative_length` second, while keeping `p_cap` diagnostic-first. The leakage check remains prompt-token-count and/or effective budget `E`, not a second probe loss.
- The first code path for that recommendation is now in-repo: `build_probe_dataset.py` can emit prompt-level probability targets for `p_loop`, `p_cap`, or `s_t` from repeated rollouts, can also emit `mean_relative_length` regression labels, the builder has task-aware prompt formatting for `GPQA` / `MMLU-Pro`, `train_probe.py` can train/evaluate against both continuous target kinds, and the builder writes one reusable repeated-rollout archive to `diagnostics/prompt_rollout_archive.jsonl`. The current implementation note is `prompt-profile-probe.md`.
- The newest recommendation note now includes the first Athena-backed objective refresh on top of the real `GPQA` pilot read. The update is important: the 2026-03-21 `32 / 16` pilot completed cleanly, but on that actual slice `s_0.9` collapsed exactly to `p(max_length_hit)` prompt by prompt and the trained probe was essentially constant-baseline quality. The recommendation now shifts one notch further: keep the prompt-profile framing, but move the next main head to direct `p_loop` rather than another thresholded tail label, with `mean_relative_length` kept as the no-plumbing backup.
