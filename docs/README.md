...
- The first code path for that recommendation is now in-repo: `build_probe_dataset.py` can emit prompt-level `s_t` soft targets from repeated rollouts, the builder now has task-aware prompt formatting for `GPQA` / `MMLU-Pro`, `train_probe.py` can train/evaluate against those soft targets, and the SLURM launcher now exposes the needed `MODEL_ID` / `TASK_KIND` / decode-policy knobs in addition to `TARGET_KIND` / `NUM_GENERATIONS` / `PROFILE_TAIL_THRESHOLD`. The current implementation note is `prompt-profile-probe.md`.
