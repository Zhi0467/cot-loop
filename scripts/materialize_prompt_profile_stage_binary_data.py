...
def _load_binary_split(
    *,
    ...
    for sample_id in sample_ids:
        ...
        stage_label = int(prompt_profile_majority_tail_label(row, threshold=tail_threshold))
        labels.append(float(stage_label))
        materialized_row = dict(row)
        materialized_row["stage_tail_threshold"] = float(tail_threshold)
        materialized_row["stage_majority_tail"] = stage_label
        materialized_row["stage_label_name"] = DEFAULT_STAGE_LABEL
