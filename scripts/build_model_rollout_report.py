...
DATASETS: tuple[DatasetInfo, ...] = (
    DatasetInfo(
        ...
        prompt_instruction=(
            "Raw problem text followed by the instruction "
    ...
    DatasetInfo(
        ...
        prompt_instruction=(
            "Question text, a shuffled A-D answer block, and the instruction that "
            "the final non-empty line must be a JSON object like "
            "'{\"answer\": \"X\"}'."
        ),
    ),
    DatasetInfo(
        ...
        prompt_instruction=(
            "Question text, an A-J answer list, and the instruction that the final "
    ...
    DatasetInfo(
        ...
        prompt_instruction=(
            "LiveCodeBench's format_prompt_generation pipeline, producing raw string "
            "prompts rather than a tokenizer chat-template wrapper."
        ),
    ),
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--report-stem",
        default="cross_dataset_rollout_report",
        help="Base filename for generated summary files and report artifacts.",
    )
    parser.add_argument(
        "--model-label",
        default=None,
        help="Optional display label for the report title; defaults to metadata.model_id.",
    )
    parser.add_argument(
        "--pdflatex",
        default="pdflatex",
...
def _load_stats_payloads(stats_dir: Path) -> list[tuple[Path, dict[str, Any]]]:
    ...
    if not payloads:
        raise FileNotFoundError(f"No stats JSON payloads found under {stats_dir}")
    return payloads
...
def _prompt_format_text(row: dict[str, Any], info: DatasetInfo) -> str:
    if info.task_kind == "livecodebench_codegen":
        ...
        return "Raw string prompt from LiveCodeBench format_prompt_generation."

    resolved = row.get("prompt_format_resolved")
    if resolved == "chat_template":
        prefix = "Tokenizer chat template with one user turn"
    elif resolved == "raw":
...
def load_dataset_record(
    ...
    expected_dataset = _normalize_local_dataset_id(info.expected_dataset)
    for path, payload in payloads:
        metadata = payload.get("metadata", {})
        if metadata.get("task_kind") != info.task_kind:
            continue
        if _normalize_local_dataset_id(metadata.get("dataset")) != expected_dataset:
            continue
        if metadata.get("config") != info.expected_dataset_config:
            continue
        if metadata.get("split") != info.expected_split:
            continue
        matches.append((path, payload))

    if not matches:
        raise FileNotFoundError(
            f"Missing stats JSON for dataset {info.display_name!r} under the "
            f"expected contract in the provided stats dir."
        )
    if len(matches) > 1:
        paths = ", ".join(str(path.name) for path, _ in matches)
        raise ValueError(
            f"Found multiple stats JSON payloads for {info.display_name!r}: {paths}"
        )
...
def build_length_plot(rows: list[dict[str, Any]], figures_dir: Path) -> Path:
    ...
    bars2 = ax.bar(
        ...
        avg_loop_len,
        width=width,
        label="Average looped generation",
        color="#b56576",
    )
    bars3 = ax.bar(
        [i + width for i in x],
        avg_wrong_len,
        width=width,
        label="Average wrong generation",
        color="#355070",
    )
        ...
        bullets.append(
            "\n".join(
                [
                    ...
                    f"correct, looped, and max-length {_count_ratio(row['correct_and_loop_and_max_hits'], row['generated'])}; "
        ...
        blocks.append(
            "\n".join(
                [
                    ...
                    rf"\textbf{{Prompt format.}} {latex_escape(row['chat_format'])}",
                ]
            )
        )
    return "\n\n".join(blocks)
...
def build_key_findings(rows: list[dict[str, Any]]) -> str:
    ...
    return "\n".join(
        [
            ...
            (
                f"Prompt-plus-generation max-length termination is most tightly coupled to looping on "
                f"{latex_escape(tightest_overlap['display_name'])}, where the "
                f"max-length-hit loop conditional reaches "
                f"{format_percent(tightest_overlap['max_length_hit_loop_fraction'])}."
            ),
            (
...
def build_tex(
    ...
    title_model = model_label or rows[0]["model_id"]
...
\title{{Cross-Dataset Rollout Statistics Report for {latex_escape(title_model)}}}
\author{{}}
...
These rows should be read as a common-policy rollout-telemetry bundle under shared multi-sample decoding, not as benchmark-optimized single-generation accuracy.
...
\item Prompt format request: \texttt{{{latex_escape(rows[0].get("prompt_format_requested"))}}}
...
The collector contract tracks the following metrics across the bundle ({latex_escape(str(len(stats_union)))} named metrics in the union):
\begin{{itemize}}[leftmargin=1.5em]
{build_tracked_stats_list(rows)}
\end{{itemize}}

The JSON payloads also retain the raw event counts for prompts, graded/generated rollouts, looped rollouts, max-length hits, prompt-too-long exclusions, and the saved overlap counts needed for pairwise and partial three-way breakdowns.
...
The length panel shows that both looped generations and wrong generations are substantially longer than the dataset-wide average wherever the loop rate is appreciable. In harder reasoning and coding settings, wrong rollouts become long well before they are reliably correct, which is consistent with the overall loop-heavy tail behavior in this sweep.
...
def main() -> None:
    ...
    payloads = _load_stats_payloads(stats_dir)
    rows = [load_dataset_record(payloads, info) for info in DATASETS]
    validate_bundle_contract(rows)
    write_summary_files(rows, out_dir)
    build_figures(rows, out_dir)
    tex_path = build_tex(rows, out_dir, args.report_stem, args.model_label)
